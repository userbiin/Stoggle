...
from constants import _KOSPI200_FALLBACK, KOSPI200_TICKERS  # noqa: F401 (re-exported)
...
app.conf.beat_schedule = {
    ...
    # 카테고리(정치/사회/경제) 뉴스 수집 (1시간) — Naver News Open API
    "crawl-category-news": {
    ...
    for ticker in KOSPI200_TICKERS:
        try:
            ...
            if scored:
                ...
                logger.info("카테고리 relevance [%s]: %d건", ticker, len(news_for_dedup[ticker]))
        except Exception as e:
...
def analyze_single_ticker(ticker: str):
    ...
    # LLM 구조화 분석 → InsightCache.keywords_json 저장
    if items:
        try:
            from agents.dedup_indexer import Article
            from agents.analysis_agent import run_and_save as analysis_save
            articles = [Article(url=i.url, title=i.title, summary=i.summary or "") for i in items]
            asyncio.run(analysis_save(articles, ticker))
        except Exception as e:
            logger.warning("analysis_agent 실패 [%s]: %s", ticker, e)

    # Redis 중복 실행 방지 lock 해제
    try:
        import redis as _redis
        r = _redis.from_url(os.getenv("REDIS_URL", "redis://localhost:6379/0"), socket_timeout=1.0)
        r.delete(f"analysis:running:{ticker}")
    except Exception:
        pass
